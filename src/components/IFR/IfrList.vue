<template>
	<div class="b-table">
			...
			<div class="field has-addons">
				<div class="control is-expanded">
					<span class="select is-fullwidth">
						<select aria-label="Please pick one to sort by - default is Infections descending" id="selectbox" v-model="selected" @change="sort(this.selected, this.orderBy)">
		...
		<div class="table-wrapper has-mobile-cards">
			<table class="table is-fullwidth is-hoverable is-striped">
				<thead>
					<tr>
						...
						<th class="ifr">
							...
							<button aria-label="Explanation for the minimum IFR - a Modal-Window will Pop Up" @click="openModal('IFR')" class="button is-primary is-outlined is-small">
						...
						<th class="aboveIoannidis">
							...
							<button aria-label="Explanation for the Above Ioannidis column - a Modal Window will Pop Up" @click="openModal('Ioannidis')" class="button is-primary is-outlined is-small">
								<i class="fas fa-info"></i>
							</button>
						</th>
					</tr>
				...
				<tbody>
					<ifr-item v-for="item in items" :key="item.key" :country="item.country" :infections="item.infections" :deaths="item.deaths" :population="item.population" :ifr="item.IFR" :above-ioannidis="item.aboveIoannidis" :cfr="item.CFR" :date="item.date" @handle="openModal"> </ifr-item>
				</tbody>
			</table>
		</div>
	</div>
	<click-modal :isActive="isActive" @handle="openModal">
		...
		<div v-else-if="mode === 'Ioannidis'" class="content" :class="{ mode }">
			...
			<p>
				I have to be very cautious now, because I don't want to offend anyone specifically. However some people came up with studies<sup><a href="https://doi.org/10.1101/2020.05.13.20101253" target="_blank">[1]</a><a href="https://doi.org/10.1111/eci.13554" target="_blank">[2]</a></sup>
				by
				<a href="https://en.wikipedia.org/wiki/John_Ioannidis" target="_blank">John P. Ioannidis</a>
				in which he comes to the conclusion that the median and/or average
				<abbr title="Infection Fatality Rate">IFR</abbr> of COVID-19 is around 0.04% and 0.15%.
			</p>
			...
			<p>
				The green checkmark (<span class="icon is-small has-text-success"><i class="fas fa-check"></i></span>) indicates that the minimum <abbr title="Infection Fatality Rate">IFR</abbr> lies above 0.15. The red cross (<span class="icon is-small has-text-danger"><i class="fas fa-times"></i></span>) indicates that the treshhold has not been reached yet.
			</p>
		</div>
	</click-modal>
</template>
